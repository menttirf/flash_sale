...
public class BusinessJobConfig {
    ...
    //seckillProduct
    @Bean(initMethod = "init")
    public SpringJobScheduler initSeckillProductCacheJob(CoordinatorRegistryCenter registryCenter, SeckillProductCacheJob productCacheJob){
        LiteJobConfiguration jobConfiguration = ElasticJobUtil.createJobConfiguration(productCacheJob.getClass(),
                productCacheJob.getCron(),
                3,
                "0=10,1=12,2=14",
                false);
        SpringJobScheduler springJobScheduler = new SpringJobScheduler(productCacheJob, registryCenter,jobConfiguration);
        return springJobScheduler;
    }
}
